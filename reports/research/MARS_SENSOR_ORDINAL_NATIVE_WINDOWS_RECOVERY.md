...
The bounded allocator amendment retains the frozen global L2 gradient clipping threshold of 2.0 for both pixel and scene updates while passing `foreach=False`. This removes the peak-memory TensorList workspace and uses per-tensor bounded temporary allocation. Model, losses, optimizer, batch and crop sizes, precision, seed, schedule, data, folds, comparators, thresholds, bootstrap, all seven gates, and scientific digest remain unchanged. No runtime smoke or held data was run by the amendment, and neither a further smoke nor a held run is authorized by it.
