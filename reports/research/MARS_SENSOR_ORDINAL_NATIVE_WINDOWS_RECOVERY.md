...
After Codex review, exactly one corrected native-Windows fitting-only `--checkpoint-roundtrip-smoke` is authorized. It uses fold-3 fitting evidence only, the production batch-16 path, and a checkpoint round trip. It must not compute inner-validation outcomes or open held data, semantic comparator values, folds 0/1/2, external evidence, protected evidence, or official evidence. No held-fold run is authorized. The corrected smoke has not been run by this amendment.