#!/usr/bin/env python3
"""One-shot fresh-negative safety test for the spatial-Prithvi ensemble."""
...
DEFAULT_PROTOCOL = Path("configs/cloudsen12_fresh_spatial_prithvi_evaluation_protocol.json")


def write_markdown(path: Path, report: dict[str, Any]) -> None:
    available = report["strata"]["available"]
    lines = [
        "# Fresh CloudSEN spatial-Prithvi safety test",
        "",
        ...
        f"- Current/candidate p95 scores: **{available['current']['raw_score']['p95']:.5f} / {available['candidate']['raw_score']['p95']:.5f}**.",
        f"- All fresh-safety gates pass: **{str(report['all_safety_gates_pass']).lower()}**.",
        "",
        "The exact MARS paper cache remained unopened.",
        "",
    ]
...
def main() -> int:
    ...
    if sha256(Path(__file__).resolve()) != protocol["evaluator"]["sha256"]:
        raise ValueError("Fresh ensemble evaluator hash mismatch")
    paths: dict[str, Path] = {}
    for name, contract in protocol["inputs"].items():
        path = (ROOT / contract["path"]).resolve()
        if sha256(path) != contract["sha256"]:
            raise ValueError(f"Frozen fresh ensemble input hash mismatch: {name}")
        paths[name] = path
    ...
    if (
        ...
        raise ValueError("Fresh ensemble row or stratum contract changed")
    ...
    development = load_development(
        {"inner": paths["inner"], "fold0": paths["fold0"], "fold1": paths["fold1"]}, paths["development_scores"]
    )
    ...
    candidate = blend_scores(spatial, prithvi, float(ensemble_control["prithvi_weight"]))
    if not all(np.isfinite(values).all() for values in (current, spatial, prithvi, candidate)):
        raise ValueError("Fresh ensemble scores contain non-finite values")
    ...
    full_bounds = {
        "full_rows": int(expected["full_rows"]), "available_rows": int(expected["available_rows"]), "unavailable_rows": unavailable,
    ...
    report = {
        "schema_version": 1, "scope": "one-shot fresh external-negative safety evaluation for spatial-Prithvi ensemble",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "strata": strata, "full_cohort_bounds": full_bounds,
        "paired_transitions": {
        ...
        "component_score_summary": {
            ...
            "ensemble_p95": float(np.quantile(candidate, 0.95)),
        },
        "safety_checks": checks, "all_safety_gates_pass": passed,
        "decision": "Authorize exact MARS paper evaluation." if passed else "Reject ensemble before exact MARS paper evaluation.",
        "paper_test_accessed": False,
        "provenance": {"protocol_sha256": sha256(protocol_path), "script_sha256": sha256(Path(__file__).resolve()), "git_commit": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip(), "device": str(torch.cuda.get_device_name(device) if device.type == "cuda" else device)},
    }
    ...
    print(json.dumps({"ok": passed, "checks": checks, "transitions": report["paired_transitions"]}, sort_keys=True))
